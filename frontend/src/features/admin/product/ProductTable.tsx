...
export function ProductTable() {
  ...
  return <section className="admin-page product-page">
    ...
    <div className="product-table-wrap"><div className="product-table-scroll"><table className="product-table"><thead><tr><th className="table-row-number">ลำดับ</th><th>รูปภาพ</th><th>ชื่อสินค้า</th><th>หมวดหมู่</th><th>ราคา</th><th>สต็อก</th><th>สถานะ</th><th className="table-display-column">แสดง</th><th>จัดการ</th></tr></thead><tbody>{productsQuery.isLoading && <tr><td colSpan={9}>กำลังโหลดสินค้า...</td></tr>}{productsQuery.isError && <tr><td colSpan={9}>ไม่สามารถโหลดสินค้าได้: {productsQuery.error.message}</td></tr>}{!productsQuery.isLoading && !productsQuery.isError && products.length === 0 && <tr><td colSpan={9}>ยังไม่มีสินค้า</td></tr>}{products.map((product, index) => <tr key={product.id}><td className="table-row-number">{(page - 1) * pageSize + index + 1}</td><td className="product-image-cell">{product.imageUrl ? <img className="product-table-image" src={product.imageUrl} alt={product.name} /> : <div className="product-image-placeholder" aria-label="ไม่มีรูปสินค้า"><ImageOff size={28} aria-hidden="true" /></div>}</td><td className="product-name"><div className="product-name-row"><strong>{product.name}</strong>{product.isRecommended && <span className="product-recommended-tag">แนะนำ</span>}</div></td><td>{product.categoryName}</td><td className="numeric">{product.salePrice} บาท</td><td>{product.stockQuantity} {product.unitName}</td><td><span className={`admin-status ${product.stockStatus === 'available' ? 'available' : 'low'}`}>{product.stockStatus === 'available' ? 'พร้อมขาย' : 'สต็อกต่ำ'}</span></td><td className="table-display-column"><button className={`product-display-status ${product.isActive ? 'is-active' : 'is-inactive'}`} type="button" disabled={updateMutation.isPending} aria-pressed={product.isActive} aria-label={`${product.isActive ? 'ปิด' : 'เปิด'}การแสดง ${product.name}`} onClick={() => toggleActive(product)}>{product.isActive ? <Check size={19} aria-hidden="true" /> : <X size={19} aria-hidden="true" />}</button></td><td><div className="product-actions"><Link className="product-edit" to={`/admin/products/${product.id}/edit`} aria-label={`แก้ไข ${product.name}`}><Pencil size={17} /></Link><button className="product-delete" type="button" disabled={deleteMutation.isPending} aria-label={`ลบ ${product.name}`} onClick={() => deleteProduct(product.id, product.name)}><Trash2 size={17} /></button></div></td></tr>)}</tbody></table></div>{meta && <AdminTablePagination currentPage={page} totalItems={meta.total} pageSize={pageSize} onPageChange={setPage} label="สินค้า" />}</div>
  </section>
}
