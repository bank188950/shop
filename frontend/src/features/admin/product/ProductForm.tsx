...
export function ProductForm({ product }: { product?: AdminProduct }) {
  ...
  return <section className="admin-page product-form-page">
    ...
    <form className="product-form-card" onSubmit={submit}>
      <div className="product-form-grid"><label>ชื่อสินค้า<Input required defaultValue={product?.name} placeholder="เช่น ลูกชิ้นหมูพรีเมียม" /></label><label>หมวดหมู่<Select value={category || undefined} onValueChange={setCategory}><SelectTrigger aria-label="หมวดหมู่สินค้า" aria-required="true"><SelectValue placeholder="เลือกหมวดหมู่" /></SelectTrigger><SelectContent><SelectItem value="ลูกชิ้น">ลูกชิ้น</SelectItem><SelectItem value="ไส้กรอก">ไส้กรอก</SelectItem><SelectItem value="เครื่องดื่ม">เครื่องดื่ม</SelectItem></SelectContent></Select></label><label>ราคา (บาท)<Input type="number" min="0" required defaultValue={product?.price} placeholder="0" /></label><label>จำนวนคงเหลือ<Input type="number" min="0" required defaultValue={product?.stock.match(/\d+/)?.[0]} placeholder="0" /></label></div>
