...
export function BannerForm({ banner }: { banner?: AdminBanner }) {
  ...
  return <section className="admin-page banner-form-page">
    ...
    <form className="banner-form-card" onSubmit={submit}>
      ...
      <div className="banner-image-upload"><ImagePlus size={27} aria-hidden="true" /><div><strong>อัปโหลดรูปภาพแบนเนอร์</strong><span>รองรับ JPG, PNG ขนาดไม่เกิน 5 MB</span><span className="banner-image-size-hint">ขนาดรูป 1800 × 900 px</span></div><Input type="file" accept="image/png,image/jpeg" aria-label="อัปโหลดรูปภาพแบนเนอร์" onChange={(event) => selectImage(event.target.files?.[0])} /></div>
