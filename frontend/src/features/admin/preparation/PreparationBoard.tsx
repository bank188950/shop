...
export function PreparationBoard() {
  ...
  async function removeOrderFromBatch(batchId: string, orderId: string) {
    ...
    const result = await Swal.fire({
      title: 'นำรายการออกจากรอบเตรียมของ?',
  ...
  return <section className="admin-page preparation-page">
    ...
    <section className="preparation-batches" aria-labelledby="preparation-batches-title"><div className="preparation-section-heading"><div><h2 id="preparation-batches-title"><Flame size={21} aria-hidden="true" />รอบเตรียมของ</h2><p>เตรียมครบแล้ว กดปุ่ม “พร้อมส่ง” เพื่อยืนยัน</p></div><span className={`preparation-period-badge ${period}`}>{deliveryPeriods[period].label}</span></div>{preparingBatches.length ? <div className="preparation-batch-grid">{preparingBatches.map((batch, index) => { const batchOrders = getBatchOrders(batch, orders); const items = getOrderItems(batchOrders); return <article key={batch.id} className="preparation-batch-card"><div className="preparation-batch-heading"><div><h3>รอบเตรียมของ {index + 1}</h3><p>สินค้าสำหรับ {batchOrders.length} คน · สร้างเมื่อ {batch.createdAt}</p></div><span className="preparation-status preparing">เตรียมของ</span></div><div className="preparation-item-summary"><h4><PackageOpen size={18} aria-hidden="true" />ของที่ต้องเตรียม</h4><ul>{items.map((item) => <li key={item.name}><span>{item.name}</span><strong>{item.quantity} {item.unit} <small>· {item.pieces} ชิ้น</small></strong></li>)}</ul></div><details><summary>ดูของที่ลูกค้าแต่ละคนสั่ง <ChevronRight size={17} aria-hidden="true" /></summary><ul className="preparation-customer-list">{batchOrders.map((order) => <li key={order.id}><div className="preparation-customer-order-header"><strong className="preparation-customer-order-id">{order.id}</strong><strong>{order.customerName}</strong><span>· {order.location}</span></div><div className="preparation-customer-order-items"><p>{order.items.map((item) => <span key={item.name}>{item.name} {item.quantity} {item.unit} · <span className="preparation-customer-item-pieces">{getItemPieceCount(item.name, item.quantity)} ชิ้น</span></span>)}</p><button type="button" aria-label={`นำ ${order.customerName} ออกจากรอบเตรียมของ`} onClick={() => removeOrderFromBatch(batch.id, order.id)}><X size={16} aria-hidden="true" />นำออก</button></div></li>)}</ul></details><button type="button" className="preparation-ready-button" onClick={() => markBatchReady(batch.id)}><Check size={18} aria-hidden="true" />เปลี่ยนสถานะ พร้อมส่ง</button></article> })}</div> : <p className="preparation-empty">ยังไม่มีรอบที่กำลังเตรียม เลือกออเดอร์จากคิวด้านบนเพื่อสร้างรอบ</p>}</section>
