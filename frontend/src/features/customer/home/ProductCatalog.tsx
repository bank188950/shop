      ...
      <div className="mt-6 grid grid-cols-4 gap-[18px] max-lg:grid-cols-2 max-md:mt-4 max-md:gap-3">
        {products.map((product) => (
          <article className={`overflow-hidden rounded-[17px] border border-[#d8dfd5] bg-white transition ${product.status === 'sold-out' ? 'bg-[#f4f5f2]' : 'hover:-translate-y-0.5 hover:shadow-xl hover:shadow-[#2d3627]/10'}`} key={product.id}>
            <div className="relative aspect-[1.24/1] overflow-hidden bg-[#eef1ec]"><img className={`size-full object-cover ${product.status === 'sold-out' ? 'grayscale saturate-[.25] opacity-40' : ''}`} src={product.image} alt={product.name} /><span className={`absolute right-3 top-3 rounded-full px-2.5 py-1 text-base font-extrabold text-white shadow-md ${product.status === 'low-stock' ? 'bg-[#d7443e]' : product.status === 'sold-out' ? 'bg-[#7f887e]' : 'bg-brand'} max-md:right-2 max-md:top-2 max-md:px-2 max-md:py-0.5`}>{product.badge}</span></div>
            <div className={`p-4 max-md:p-3 ${product.status === 'sold-out' ? 'text-[#9ea49d]' : ''}`}><h3 className="m-0 font-heading text-xl leading-tight text-[#26352d] max-md:text-lg">{product.name}</h3><p className={`mt-1 mb-0 text-lg font-extrabold ${product.status === 'low-stock' ? 'text-[#d7443e]' : product.status === 'sold-out' ? 'text-[#9ea49d]' : 'text-brand'} max-md:text-base`}>{product.stockLabel}</p><div className="mt-5 flex items-center justify-between"><strong className={`font-heading text-xl ${product.status === 'sold-out' ? 'text-[#9ea49d]' : 'text-[#236f33]'}`}>฿{product.price}</strong><button className={`grid size-[42px] place-items-center rounded-full border-0 text-white transition active:scale-95 max-md:size-9 ${product.status === 'sold-out' ? 'cursor-not-allowed bg-[#b7bcb5]' : 'bg-brand hover:bg-brand-dark'}`} type="button" disabled={product.status === 'sold-out'} onClick={() => addItem({ productId: product.id, quantity: 1 })} aria-label={product.status === 'sold-out' ? `${product.name} สินค้าหมด` : `เพิ่ม ${product.name} ลงตะกร้า`}>{product.status === 'sold-out' ? <Ban size={22} /> : <Plus size={22} />}</button></div></div>
          </article>
        ))}
      </div>
    </section>
