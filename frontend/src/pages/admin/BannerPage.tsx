import { Check, ChevronDown, ChevronUp, ImageOff, Pencil, Plus, Trash2, X } from 'lucide-react'
...
import { useBanners, useDeleteBanner, useMoveBanner, useSaveBanner } from '@/features/admin/banner/hooks/useBanners'
...
export function BannerPage() {
  ...
  return <section className="admin-page banner-page">
    ...
    <div className="product-table-wrap banner-table-wrap"><div className="product-table-scroll"><table className="product-table banner-table"><thead><tr><th className="table-row-number">ลำดับ</th><th>รูปภาพ</th><th>หัวข้อ</th><th>จัดลำดับ</th><th>แสดง</th><th>จัดการ</th></tr></thead><tbody>{bannersQuery.isLoading && <tr><td colSpan={6}>กำลังโหลดแบนเนอร์...</td></tr>}{bannersQuery.isError && <tr><td colSpan={6}>ไม่สามารถโหลดแบนเนอร์ได้: {bannersQuery.error.message}</td></tr>}{!bannersQuery.isLoading && !bannersQuery.isError && visibleBanners.length === 0 && <tr><td colSpan={6}>ยังไม่มีแบนเนอร์</td></tr>}{visibleBanners.map((banner, index) => <tr key={banner.id}><td className="table-row-number">{(page - 1) * pageSize + index + 1}</td><td>{banner.imageUrl ? <img className="banner-table-image" src={banner.imageUrl} alt={banner.title} /> : <div className="banner-image-placeholder" aria-label="ไม่มีรูปแบนเนอร์"><ImageOff size={26} aria-hidden="true" /></div>}</td><td><strong>{banner.title}</strong></td><td><div className="banner-order-actions"><button type="button" disabled={moveMutation.isPending || banner.id === banners[0]?.id} aria-label={`เลื่อน ${banner.title} ขึ้น`} onClick={() => moveMutation.mutate({ bannerId: banner.id, direction: 'up' })}><ChevronUp size={18} aria-hidden="true" /></button><button type="button" disabled={moveMutation.isPending || banner.id === banners[banners.length - 1]?.id} aria-label={`เลื่อน ${banner.title} ลง`} onClick={() => moveMutation.mutate({ bannerId: banner.id, direction: 'down' })}><ChevronDown size={18} aria-hidden="true" /></button></div></td><td><button className={`product-display-status ${banner.isActive ? 'is-active' : 'is-inactive'}`} type="button" disabled={saveMutation.isPending} aria-pressed={banner.isActive} aria-label={`${banner.isActive ? 'ปิด' : 'เปิด'}การแสดง ${banner.title}`} onClick={() => toggleBannerActive(banner.id)}>{banner.isActive ? <Check size={19} aria-hidden="true" /> : <X size={19} aria-hidden="true" />}</button></td><td><div className="product-actions"><Link className="product-edit" to={`/admin/banners/${banner.id}/edit`} aria-label={`แก้ไขแบนเนอร์ ${banner.title}`}><Pencil size={17} aria-hidden="true" /></Link><button className="product-delete" type="button" disabled={deleteMutation.isPending} onClick={() => deleteBanner(banner.id, banner.title)} aria-label={`ลบแบนเนอร์ ${banner.title}`}><Trash2 size={17} aria-hidden="true" /></button></div></td></tr>)}</tbody></table></div><AdminTablePagination currentPage={page} totalItems={banners.length} pageSize={pageSize} onPageChange={setPage} label="แบนเนอร์" /></div>
  </section>
}
