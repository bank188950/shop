...
export function OrderPage() {
  ...
  return <section className="admin-page">
    ...
    <div className="admin-table-wrap"><div className="admin-table-scroll"><table className="admin-data-table"><thead><tr><th>เลขที่รายการสั่งซื้อ</th><th>ลูกค้า / จุดรับ</th><th>รอบส่ง</th><th>รายการ</th><th>ยอดรวม</th><th>ชำระเงิน</th><th>สถานะ</th><th><span className="sr-only">ดูรายละเอียด</span></th></tr></thead><tbody>{ordersQuery.isLoading ? <tr><td className="admin-empty-cell" colSpan={8}>กำลังโหลดรายการสั่งซื้อ...</td></tr> : ordersQuery.isError ? <tr><td className="admin-empty-cell" colSpan={8}>ไม่สามารถโหลดรายการสั่งซื้อได้: {ordersQuery.error.message}</td></tr> : orders.length ? visibleOrders.map((order) => <tr key={order.id}><td><strong className="admin-order-number">{order.orderNumber}</strong><small>{orderedAtLabel(order.orderedAt)}</small></td><td><strong>{order.userName}</strong><small>{order.locationName}</small></td><td><strong>{deliveryPeriods[order.deliveryPeriod].label}</strong><small>{deliveryPeriods[order.deliveryPeriod].deliveryTime}</small></td><td>{order.items.map((item) => <small key={item.name}>{item.name} {item.quantity} {item.unitName}</small>)}</td><td className="numeric"><strong>{formatPrice(order.totalAmount)}</strong></td><td><span className={`admin-status ${paymentStatusClass(order.paymentStatus)}`}>{paymentStatusLabel(order.paymentStatus)}</span></td><td><span className={`admin-status ${orderStatusClass(order.orderStatus)}`}>{orderStatusLabel(order.orderStatus)}</span></td><td><Link className="admin-table-link" to={`/admin/orders/${order.id}`} aria-label={`ดูรายละเอียดรายการสั่งซื้อ ${order.orderNumber}`}><Eye size={19} aria-hidden="true" /></Link></td></tr>) : <tr><td className="admin-empty-cell" colSpan={8}>ไม่พบรายการสั่งซื้อที่ตรงกับตัวกรอง</td></tr>}</tbody></table></div><AdminTablePagination currentPage={page} totalItems={orders.length} pageSize={pageSize} onPageChange={setPage} label="รายการสั่งซื้อ" /></div>
  </section>
}
