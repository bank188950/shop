...
repository แยกเป็น SPA ใน `frontend/`, PHP API ใน `backend/` และ schema MySQL ใน `specs/database/schema.sql` React ใช้ Axios และ React Query เรียก API ที่มี route ธุรกิจจริง; ข้อมูลแคตตาล็อก, บัญชี, คำสั่งซื้อ และการจัดเตรียมเก็บใน MySQL ขณะที่ตะกร้ายังเก็บในเบราว์เซอร์เพื่อคงรายการก่อน checkout.
...
| คำสั่งซื้อ, การชำระเงิน, สต็อก | `orders`, `order_items`, `order_payments`, `products` | สร้าง order ใน transaction และ lock สินค้าก่อนตัดสต็อก |
