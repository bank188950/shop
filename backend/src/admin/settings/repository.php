...
function settings_find(PDO $db): array { settings_ensure_row($db); return $db->query('SELECT morning_order_cutoff, morning_delivery_start, morning_delivery_end, afternoon_order_cutoff, afternoon_delivery_start, afternoon_delivery_end, notice_popup_message, is_notice_popup_enabled FROM settings WHERE id = 1')->fetch(); }
...
function settings_to_api(array $settings, array $ads): array {
    return ['morningCutoff' => substr($settings['morning_order_cutoff'], 0, 5), 'morningDelivery' => substr($settings['morning_delivery_start'], 0, 5) . '–' . substr($settings['morning_delivery_end'], 0, 5), 'afternoonCutoff' => substr($settings['afternoon_order_cutoff'], 0, 5), 'afternoonDelivery' => substr($settings['afternoon_delivery_start'], 0, 5) . '–' . substr($settings['afternoon_delivery_end'], 0, 5), 'noticeMessage' => $settings['notice_popup_message'] ?? '', 'isNoticePopupEnabled' => (bool) $settings['is_notice_popup_enabled'], 'advertisementTexts' => array_map(static fn ($ad) => $ad['message'], $ads), 'isAdvertisementVisible' => (bool) array_filter($ads, static fn ($ad) => $ad['is_active'])];
}
function settings_save(PDO $db, array $data): void {
    ...
    $statement->execute(array_intersect_key($data, array_flip(['morning_order_cutoff', 'morning_delivery_start', 'morning_delivery_end', 'afternoon_order_cutoff', 'afternoon_delivery_start', 'afternoon_delivery_end', 'notice_popup_message', 'is_notice_popup_enabled'])));
