...
function settings_find(PDO $db): array { settings_ensure_row($db); return $db->query('SELECT morning_order_cutoff, morning_delivery_start, morning_delivery_end, afternoon_order_cutoff, afternoon_delivery_start, afternoon_delivery_end, notice_popup_message, is_notice_popup_enabled, is_badge_notification_enabled FROM settings WHERE id = 1')->fetch(); }
