...
function settings_to_api(array $settings, array $ads): array {
    return ['morningCutoff' => substr($settings['morning_order_cutoff'], 0, 5), 'morningDelivery' => substr($settings['morning_delivery_start'], 0, 5) . '–' . substr($settings['morning_delivery_end'], 0, 5), 'afternoonCutoff' => substr($settings['afternoon_order_cutoff'], 0, 5), 'afternoonDelivery' => substr($settings['afternoon_delivery_start'], 0, 5) . '–' . substr($settings['afternoon_delivery_end'], 0, 5), 'noticeMessage' => $settings['notice_popup_message'] ?? '', 'isNoticePopupEnabled' => (bool) $settings['is_notice_popup_enabled'], 'isBadgeNotificationEnabled' => (bool) $settings['is_badge_notification_enabled'], 'isSlipQuotaAlertEnabled' => (bool) $settings['is_slip_quota_alert_enabled'], 'advertisementTexts' => array_map(static fn ($ad) => $ad['message'], $ads), 'isAdvertisementVisible' => (bool) array_filter($ads, static fn ($ad) => $ad['is_active'])];
}
function settings_save(PDO $db, array $data): void {
    $statement = $db->prepare('UPDATE settings SET morning_order_cutoff = :morning_order_cutoff, morning_delivery_start = :morning_delivery_start, morning_delivery_end = :morning_delivery_end, afternoon_order_cutoff = :afternoon_order_cutoff, afternoon_delivery_start = :afternoon_delivery_start, afternoon_delivery_end = :afternoon_delivery_end, notice_popup_message = :notice_popup_message, is_notice_popup_enabled = :is_notice_popup_enabled, is_badge_notification_enabled = :is_badge_notification_enabled, is_slip_quota_alert_enabled = :is_slip_quota_alert_enabled WHERE id = 1');
    $statement->execute(array_intersect_key($data, array_flip(['morning_order_cutoff', 'morning_delivery_start', 'morning_delivery_end', 'afternoon_order_cutoff', 'afternoon_delivery_start', 'afternoon_delivery_end', 'notice_popup_message', 'is_notice_popup_enabled', 'is_badge_notification_enabled', 'is_slip_quota_alert_enabled'])));
