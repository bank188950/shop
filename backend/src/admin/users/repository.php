...
{
    $statement = $db->prepare("INSERT INTO users (role, full_name, phone, line_account, password_hash, default_location_id, is_active) VALUES ('user', :full_name, :phone, :line_account, :password_hash, :default_location_id, :is_active)");
...
function user_update(PDO $db, int $id, array $data): void
{
    ...
    $statement = $db->prepare($sql . ' WHERE id = :id AND role = \'user\'');
    $statement->execute($params);
}

function user_remove(PDO $db, int $id): void
{
    user_message_delete_user_images($db, $id);
