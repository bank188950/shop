...
function customer_order_to_api(array $order, array $items): array
{
    return [
        'id' => (int) $order['id'],
...
function customer_order_lock_product(PDO $db, int $productId): ?array
{
...
function customer_order_change_stock(PDO $db, array $product, int $quantity, int $sign): void
{
...
function customer_order_insert(PDO $db, int $customerId, array $data, array $lines, float $total): int
{
    $statement = $db->prepare('INSERT INTO orders (order_number, customer_id, location_id, delivery_date, delivery_period, subtotal_amount, total_amount, customer_note)
        VALUES (:order_number, :customer_id, :location_id, :delivery_date, :delivery_period, :subtotal, :total, :customer_note)');
    $statement->execute([
    ...
    $number->execute(['order_number' => sprintf('LL-%s-%04d', date('ymd'), $orderId), 'id' => $orderId]);
...
function customer_order_find(PDO $db, int $orderId, ?int $customerId = null): ?array
{
...
function customer_order_items(PDO $db, int $orderId): array
{
...
function customer_order_list(PDO $db, int $customerId): array
{
    $statement = $db->prepare('SELECT o.*, l.name AS location_name FROM orders o INNER JOIN locations l ON l.id = o.location_id WHERE o.customer_id = :customer_id ORDER BY o.ordered_at DESC, o.id DESC');
    $statement->execute(['customer_id' => $customerId]);
    return array_map(static fn (array $order) => customer_order_to_api($order, customer_order_items($db, (int) $order['id'])), $statement->fetchAll());
}

function customer_order_mark_paid(PDO $db, int $orderId): void
{
...
function customer_order_cancel(PDO $db, int $orderId, string $reason): void
{
    $db->beginTransaction();
    try {
        foreach (customer_order_items($db, $orderId) as $item) {
            if (!$item['product_id']) continue;
            $product = customer_order_lock_product($db, (int) $item['product_id']);
            if ($product) customer_order_change_stock($db, $product, (int) $item['quantity'], 1);
        }
...
function customer_order_expire_pending(PDO $db): void
{
    ...
    foreach ($orders as $order) {
        $expiresAt = strtotime($order['ordered_at']) + CUSTOMER_ORDER_PAYMENT_MINUTES * 60;
        $pastCutoff = $order['delivery_date'] < $today
            || ($order['delivery_date'] === $today && $now > customer_order_period_cutoff($settings, $order['delivery_period']));
        if (time() < $expiresAt && !$pastCutoff) continue;
        customer_order_cancel($db, (int) $order['id'], 'ไม่ได้ชำระเงินภายในเวลาที่กำหนด');
    }
}
